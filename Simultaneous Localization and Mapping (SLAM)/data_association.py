...
class Solution(Bot):
    ...
    def data_association(self, measurements, true_idx, current_map):
        ...
        # ── Adaptive gate ──────────────────────────────────────────────────────
        # Base term: 3-sigma noise envelope in 2-D (chi2 dof=2 at 95% ≈ 2.45σ,
        # we use 3.0 for a little extra margin).
        # Speed term: at high speed the dead-reckoning position drifts between
        # frames, so cone projections land further from their true map position.
        # We add a fraction of the distance the car travels in one DT so the
        # gate grows gracefully with velocity rather than snapping hard at 2.95m.
        ...
        # For every map landmark, check whether it was matched this frame.
        # Consistent match  → increment its hit counter.
        # No match this frame → reset counter to 0 (streak broken).
        # Only associations whose counter has reached CONFIRM_THRESH are
        # considered confirmed; the rest are set back to -1 so they are
        # invisible to both the visualiser and the mapping step.
        ...
        # build confirmed assoc — only expose associations that have been
        # consistently matched for at least CONFIRM_THRESH frames in a row
